/** Иконки только через Vite-import → URL с content-hash; public/ без дублей PNG. */
import splashLogo from '../assets/splash-logo.png'
import modeStuck from '../assets/icons/mode-stuck-hq.png'
import modeNoise from '../assets/icons/mode-noise-hq.png'
import flashStep from '../assets/icons/flash-step-hq.png'
import tabUnfreeze from '../assets/icons/icon-tab-unfreeze.png'
import tabWins from '../assets/icons/icon-tab-wins.png'

export const images = {
  hero: splashLogo,
  modeStuck,
  modeNoise,
  flashStep,
  tabUnfreeze,
  tabWins,
} as const

export const modeImages = {
  stuck: images.modeStuck,
  noise: images.modeNoise,
} as const

/** Для inline boot-splash в index.html (тот же файл, что hero). */
export const bootSplashLogo = splashLogo
