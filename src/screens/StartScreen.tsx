...
function isClearTask(text: string): boolean {
  ...
  // «не могу встать/сесть/пойти…» — паралич старта, не чек-лист по задаче
  if (/не\s+могу\s+(встать|сесть|начать|приступить|заставить|пойти|выходить|открыть)/i.test(t)) {
    return false
  }
  const verb =
